...
class HoyEndpointOrderingTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.activity = create_actividad(self.user, "Actividad prueba", "Curso Pruena", "otro", date.today() + timedelta(days=30))

        today = date.today()

        # asignar tareas vencidas 
        self.v_old_near = create_tarea(self.activity, "Vencida reciente", today - timedelta(days=1), 2.0)
        self.v_old_far = create_tarea(self.activity, "Vencida antigua", today - timedelta(days=5), 1.0)

        # asignar tareas para hoy
        self.t1 = create_tarea(self.activity, "Hoy esfuerzo3", today, 3.0,)
        self.t2 = create_tarea(self.activity, "Hoy esfuerzo1", today, 1.0)

        # asignar tareas próximas 
        self.p_far = create_tarea(self.activity, "Proxima lejana", today + timedelta(days=4), 2.0)
        self.p_near = create_tarea(self.activity, "Proxima cercana", today + timedelta(days=1), 1.0)
...
class HoyEndpointFiltersAndMessagesTests(APITestCase):
    ...
    def test_hoy_activity_no_tasks_with_filter(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assert_empty_response(resp, "No se encontraron tareas con los filtros aplicados")
    ...
    def test_hoy_filter_by_estado(self):
        ...
        self.t1 = create_tarea(self.act, "TareaPendiente", date.today(), 1, estado="pendiente")
